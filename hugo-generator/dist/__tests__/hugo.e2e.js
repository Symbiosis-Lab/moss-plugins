 ...
 */
async function createHugoStructureForTest(projectPath, projectInfo, runtimeDir) {
...
function copyContentFolder(src, dst) {
    fs.mkdirSync(dst, { recursive: true });
    const entries = fs.readdirSync(src, { withFileTypes: true });
    for (const entry of entries) {
        const srcPath = path.join(src, entry.name);
        const dstName = entry.name.toLowerCase() === "index.md" ? "_index.md" : entry.name;
...
function createHugoConfigForTest(runtimeDir, siteName = "Site", baseUrl = "/") {
...
function createDefaultLayoutsForTest(runtimeDir) {
...
describe.skipIf(!hugoAvailable)("Hugo Generator E2E", () => {
    ...
    describe("Flat Site (no collections)", () => {
        it("builds flat site with homepage and root pages", async () => {
            ...
            await createHugoStructureForTest(FLAT_SITE_FIXTURE, projectInfo, runtimeDir);
            createHugoConfigForTest(runtimeDir, "Flat Site Test");
            createDefaultLayoutsForTest(runtimeDir);
            // Verify structure was created
            expect(fs.existsSync(path.join(runtimeDir, "content", "_index.md"))).toBe(true);
            expect(fs.existsSync(path.join(runtimeDir, "content", "about.md"))).toBe(true);
            ...
            expect(fs.existsSync(path.join(outputDir, "contact", "index.html"))).toBe(true);
            // Verify content is rendered
            const homeHtml = fs.readFileSync(path.join(outputDir, "index.html"), "utf-8");
    ...
    describe("Collections Site (with posts folder)", () => {
        it("builds site with collections and nested content", async () => {
            ...
            await createHugoStructureForTest(COLLECTIONS_SITE_FIXTURE, projectInfo, runtimeDir);
            createHugoConfigForTest(runtimeDir, "Collections Site Test");
            createDefaultLayoutsForTest(runtimeDir);
            // Verify structure - posts/index.md should become posts/_index.md
            expect(fs.existsSync(path.join(runtimeDir, "content", "_index.md"))).toBe(true);
            expect(fs.existsSync(path.join(runtimeDir, "content", "posts", "_index.md"))).toBe(true);
            ...
            expect(fs.existsSync(path.join(outputDir, "posts", "post-1", "index.html"))).toBe(true);
            expect(fs.existsSync(path.join(outputDir, "posts", "post-2", "index.html"))).toBe(true);
            // Verify content is rendered
            const postsHtml = fs.readFileSync(path.join(outputDir, "posts", "index.html"), "utf-8");
            expect(postsHtml).toContain("Posts");
            const post1Html = fs.readFileSync(path.join(outputDir, "posts", "post-1", "index.html"), "utf-8");
...
//# sourceMappingURL=hugo.e2e.js.map