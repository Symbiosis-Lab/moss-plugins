/**
 * Unit tests for Eleventy Generator Plugin
 */
...
describe("Eleventy Generator Plugin", () => {
  ...
  beforeEach(() => {
    ...
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "eleventy-main-test-"));
  ...
  describe("on_build", () => {
    it("calls Eleventy with correct arguments", async () => {
      // Mock npm install
      ...
      expect(result.message).toBe("Eleventy build complete");
    });
    ...
    it("returns failure when Eleventy fails with error output", async () => {
      ctx.binaryConfig.setResult("npm", {
        success: true,
        exitCode: 0,
        stdout: "",
      ...
      ctx.binaryConfig.setResult("npx", {
        ...
        exitCode: 1,
        stdout: "",
        stderr: "Error: Cannot find module '@11ty/eleventy'",
      });
      ...
      expect(result.message).toContain("Eleventy build failed");
    });
    ...
    it("returns failure when npm install fails", async () => {
      ...
      expect(result.success).toBe(false);
      expect(result.message).toContain("Failed to install Eleventy dependencies");
    });
    ...
    it("reports progress during build", async () => {
      ...
      expect(reportProgress).toHaveBeenCalledWith(
        ...
        1,
        4,
        "Creating Eleventy structure..."
      );
      expect(reportProgress).toHaveBeenCalledWith(
        "building",
        2,
        4,
        "Installing Eleventy dependencies..."
      );
      expect(reportProgress).toHaveBeenCalledWith(
        "complete",
        4,
    ...
    it("cleans up runtime directory after build", async () => {
      ...
      const runtimeDir = path.join(
        mossDir,
        "plugins/eleventy-generator/.runtime"
      );
      expect(fs.existsSync(runtimeDir)).toBe(false);
    });

    it("handles content folders for collections", async () => {
      ctx.binaryConfig.setResult("npm", {
        success: true,
        exitCode: 0,
        stdout: "",
  ...
  describe("plugin registration", () => {
    ...
    it("exports default EleventyGenerator object", async () => {
      const module = await import("../main");
