/**
 * Content conversion: Substack HTML ↔ Markdown.
 *
 * Substack's body_html uses standard semantic HTML:
 * p, h2, h3, h4, strong, em, s, code, ul, ol, li, blockquote, a, img
 *
 * Headings contain anchor link buttons injected by Substack — these must be
 * stripped during conversion.
 */

/**
 * Convert Substack article HTML to Markdown.
 */
export function htmlToMarkdown(html: string): string {
  const parser = new DOMParser();
  const doc = parser.parseFromString(html, "text/html");

  // Strip Substack's anchor link buttons from headings
  doc.querySelectorAll(".header-anchor-parent").forEach((el) => el.remove());
...
/**
 * Convert Markdown to HTML suitable for Substack's ProseMirror editor.
