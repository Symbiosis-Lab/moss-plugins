  ...
  describe("deployViaGitPush", () => {
    ...
    describe("CNAME injection for custom domains", () => {
      it("injects CNAME file into gh-pages tree when domain is provided", async () => {
        ...
        // Verify mktree was called with entries including CNAME
        ...
        // Verify commit-tree used the NEW tree (with CNAME), not the original
        expect(mockExecuteBinary).toHaveBeenCalledWith(
          expect.objectContaining({
            binaryPath: "git",
      ...
      it("uses original tree when no domain is provided", async () => {
        setupFullDeployMocks();
        ...
        // Verify commit-tree used the ORIGINAL site tree
        expect(mockExecuteBinary).toHaveBeenCalledWith(
          expect.objectContaining({
            binaryPath: "git",
            args: ["commit-tree", "aaa111bbb222", "-m", "Deploy site\n\nGenerated by moss"],
        ...
        // Verify no hash-object or mktree calls
        const hashObjCalls = mockExecuteBinary.mock.calls.filter(
          (call) => call[0].binaryPath === "git" && call[0].args[0] === "hash-object"
        );
        expect(hashObjCalls).toHaveLength(0);
      });

      it("falls back to original tree when hash-object fails", async () => {
        const SITE_TREE = "aaa111bbb222";
        ...
        mockExecuteBinary
          ...
          .mockResolvedValueOnce(gitResult(true, "abc1234\n"))       // rev-parse --short HEAD
          .mockResolvedValueOnce(gitResult(true, SITE_TREE + "\n"))  // rev-parse HEAD:.moss/site
          .mockResolvedValueOnce(gitResult(false))                   // hash-object FAILS
          .mockResolvedValueOnce(gitResult(true, "ccc333ddd444\n"))  // commit-tree (original tree)
        ...
        // Verify commit-tree used the ORIGINAL tree (fallback)
        expect(mockExecuteBinary).toHaveBeenCalledWith(
          expect.objectContaining({
            binaryPath: "git",
            args: ["commit-tree", SITE_TREE, "-m", "Deploy site\n\nGenerated by moss"],
          })
        );
      });
    });
