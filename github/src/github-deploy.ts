...
export async function deployViaGitPush(options: DeployViaGitPushOptions): Promise<string> {
  ...
  // Inject CNAME file into tree if custom domain is configured.
  // Without this, each force-push to gh-pages removes the CNAME that
  // GitHub Pages needs to serve the custom domain.
  let treeSha = tree.stdout.trim();
  if (options.domain) {
    const hashObj = await executeBinary({
      binaryPath: options.gitPath,
    ...
    if (hashObj.success) {
      ...
      if (lsTree.success) {
        const treeEntries = lsTree.stdout.trimEnd()
          + "\n100644 blob " + hashObj.stdout.trim() + "\tCNAME\n";
