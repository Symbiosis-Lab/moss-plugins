...
- `[matters]` `1.4.8`: equations survive syndication — multi-line display math keeps its line structure (hoisted into `<pre><code>`, the only whitespace-preserving container Matters keeps) and a LaTeX `%` comment no longer swallows the rest of the formula. Also fixes the paragraph-shape mismatch that had silently disabled the image, audio and display-math hoists in production. See [matters/CHANGELOG.md](./matters/CHANGELOG.md).
- `[matters]` `1.4.7`: renamed collection folders are no longer re-created on every sync — collections now carry identity (known-id gate from plugin config, `syndicated:` collection-URL marker on new collection files, member-location placement for new articles); `parseFrontmatter` now reads moss-normalized (serde_yaml) frontmatter so stamped files keep their `syndicated:` identity. See [matters/CHANGELOG.md](./matters/CHANGELOG.md).
