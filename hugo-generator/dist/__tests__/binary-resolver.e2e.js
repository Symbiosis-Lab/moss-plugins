 ...
 */
function shouldRunDownloadTests() {
...
describe("Binary Resolution E2E", () => {
    ...
    describe("Download URL Construction", () => {
        ...
        it("constructs correct download URL for linux-amd64", () => {
            const version = "0.139.0";
            const url = "hugo_extended_{version}_linux-amd64.tar.gz".replace(/{version}/g, version);
            expect(url).toBe("hugo_extended_0.139.0_linux-amd64.tar.gz");
        });
        it("constructs correct download URL for windows-amd64 (zip)", () => {
            const version = "0.139.0";
            const url = "hugo_extended_{version}_windows-amd64.zip".replace(/{version}/g, version);
    ...
    describe("GitHub API Integration", () => {
        describe.skipIf(!shouldRunDownloadTests())("release fetching", () => {
            it("fetches latest Hugo release from GitHub", async () => {
                ...
                const response = await fetch("https://api.github.com/repos/gohugoio/hugo/releases/latest", {
                    headers: {
    ...
    describe("Archive Extraction", () => {
        ...
        describe.skipIf(platform.os !== "windows")("zip extraction on Windows", () => {
            it("can extract zip archives with PowerShell", () => {
                ...
                execSync(`powershell -Command "Compress-Archive -Path '${testDir}\\*' -DestinationPath '${archivePath}'"`, { stdio: "pipe" });
                // Extract
    ...
    describe("Full Download Flow", () => {
        describe.skipIf(!shouldRunDownloadTests())("Hugo download and execution", () => {
            it("downloads Hugo, extracts it, and verifies it works", async () => {
                ...
                fs.mkdirSync(binDir);
                // Get latest release
                const releaseResponse = await fetch("https://api.github.com/repos/gohugoio/hugo/releases/latest", {
                    headers: {
                        Accept: "application/vnd.github.v3+json",
                        "User-Agent": "moss-hugo-generator-test",
                    },
                ...
                if (platform.os === "windows") {
                    execSync(`powershell -Command "Invoke-WebRequest -Uri '${downloadUrl}' -OutFile '${archivePath}'"`, { stdio: "pipe", timeout: 300000 });
                }
                else {
                ...
                if (archiveFormat === "tar.gz") {
                    ...
                    execSync(`powershell -Command "Expand-Archive -Path '${archivePath}' -DestinationPath '${binDir}' -Force"`, { stdio: "pipe" });
                }
                // Find Hugo binary
                const hugoBinary = platform.os === "windows"
    ...
    describe("Integration with Hugo Generator", () => {
        describe.skipIf(!hugoAvailable)("using system Hugo", () => {
            it("can build a simple site with detected Hugo", () => {
                ...
                fs.writeFileSync(path.join(contentDir, "_index.md"), "---\ntitle: Home\n---\n# Hello World");
                // Create minimal config
                fs.writeFileSync(path.join(siteDir, "hugo.toml"), 'baseURL = "/"\ntitle = "Test"');
                // Create minimal template
                fs.writeFileSync(path.join(layoutsDir, "baseof.html"), "<!DOCTYPE html><html><body>{{ block \"main\" . }}{{ end }}</body></html>");
                fs.writeFileSync(path.join(layoutsDir, "list.html"), '{{ define "main" }}{{ .Content }}{{ end }}');
                fs.writeFileSync(path.join(siteDir, "layouts", "index.html"), '{{ define "main" }}{{ .Content }}{{ end }}');
                // Build
                const result = spawnSync("hugo", ["--source", siteDir, "--destination", outputDir, "--quiet"], { encoding: "utf-8", stdio: "pipe" });
                expect(result.status).toBe(0);
...
describe("Test Configuration", () => {
    it("reports test environment", () => {
        ...
        console.log(`   Download tests: ${shouldRunDownloadTests() ? "enabled" : "disabled (set TEST_DOWNLOAD_HUGO=true to enable)"}`);
        console.log("");
    });
});
//# sourceMappingURL=binary-resolver.e2e.js.map