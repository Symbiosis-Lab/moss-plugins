/**
 * Unit tests for Astro Generator Plugin
 */
...
describe("Astro Generator Plugin", () => {
  ...
  beforeEach(() => {
    ...
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "astro-main-test-"));
  ...
  describe("on_build", () => {
    it("calls Astro with correct arguments", async () => {
      // Mock npm install
      ...
      expect(result.message).toBe("Astro build complete");
    });
    ...
    it("returns failure when Astro fails with error output", async () => {
      ctx.binaryConfig.setResult("npm", {
        success: true,
        exitCode: 0,
        stdout: "",
      ...
      ctx.binaryConfig.setResult("npx", {
        ...
        exitCode: 1,
        stdout: "",
        stderr: "Error: Cannot find module 'astro'",
      });
    ...
    it("returns failure when npm install fails", async () => {
      ...
      expect(result.success).toBe(false);
      expect(result.message).toContain("Failed to install Astro dependencies");
    });
    ...
    it("cleans up runtime directory after build", async () => {
      ...
      const runtimeDir = path.join(
        mossDir,
        "plugins/astro-generator/.runtime"
      );
  ...
  describe("plugin registration", () => {
    ...
    it("exports default AstroGenerator object", async () => {
      const module = await import("../main");
