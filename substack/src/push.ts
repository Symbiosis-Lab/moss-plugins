/**
 * Push articles to Substack.
 *
 * Substack doesn't have a public write API. Content is pushed via the editor
 * using DOM automation (clipboard paste or execCommand).
 *
...
import { getPublicationUrl } from "./api";
 ...
 * @returns The Substack draft URL, or null if push failed
 */
export async function pushArticle(content: string): Promise<string | null> {
  ...
  // Open the Substack editor in the action panel
  const pubUrl = getPublicationUrl();
  const editorUrl = `${pubUrl}/publish/post`;
...
/**
 * JS script that runs inside the Substack editor page context.
 * This is the same script whether injected via Playwright or Tauri.
 *
 * @param title - Article title
 * @param subtitle - Article subtitle (optional)
...
(async () => {
  // Fill title
  const titleEl = document.querySelector('[placeholder="Title"]');
  if (titleEl) {
  ...
  // Fill subtitle
  const subtitleEl = document.querySelector('[placeholder*="subtitle" i]');
  if (subtitleEl && ${JSON.stringify(subtitle)}) {
    subtitleEl.value = ${JSON.stringify(subtitle)};
  ...
  // Paste HTML into the ProseMirror editor
  const pm = document.querySelector('.ProseMirror');
  if (pm) {
    pm.focus();
    // Method 1: execCommand (works in Substack's editor)
