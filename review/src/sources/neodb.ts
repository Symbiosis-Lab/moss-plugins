...
function extractExternalUrls(
  ...
  neodbUrl: string
): ReviewItem["external_urls"] {
  const urls: ReviewItem["external_urls"] = { neodb: neodbUrl };
...
/**
 * Normalize a raw NeoDB API response into a unified ReviewItem.
 */
function normalize(raw: any, base: string, originalUrl: string): ReviewItem {
  // Resolve cover URL
 ...
 */
export async function fetchFromNeoDB(url: string): Promise<ReviewItem | null> {
