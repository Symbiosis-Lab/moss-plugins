...
- Fixed: `parseFrontmatter` now reads serde_yaml-normalized frontmatter — unindented `- item` list entries, single- and double-quoted values in both scalars and list items, with YAML `''` → `'` unescaping — in addition to the plugin's own `  - "item"` emission. moss's uid stamping rewrites every synced file into the normalized form, which made `syndicated:` arrays invisible to the plugin — silently disabling article dedup, folder detection, and comment fetch for stamped files.
